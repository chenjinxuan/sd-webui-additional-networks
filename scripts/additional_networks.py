    ...
    def ui(self, is_img2img):
        ...
        with gr.Group():
            with gr.Accordion("Additional Networks", open=False):
                with gr.Row():
                    enabled = gr.Checkbox(elem_id=tabname+"_ext_an_enabled",label="Enable", value=False)
                    ctrls.append(enabled)
                    self.infotext_fields.append((enabled, "AddNet Enabled"))
                    separate_weights = gr.Checkbox(elem_id=tabname+"_ext_an_separate",label="Separate UNet/Text Encoder weights", value=False)
                    ctrls.append(separate_weights)
                    self.infotext_fields.append((separate_weights, "AddNet Separate Weights"))

                for i in range(MAX_MODEL_COUNT):
                    with FormRow(variant="compact"):
                        module = gr.Dropdown(["LoRA"],elem_id=f"{tabname}_ext_an_network_module_{i+1}", label=f"Network module {i+1}", value="LoRA")
                        model = gr.Dropdown(list(lora_models.keys()),elem_id=f"{tabname}_ext_an_model_{i+1}",  label=f"Model {i+1}", value="None")
                        with gr.Row(visible=False):
                            model_path = gr.Textbox(value="None",elem_id=f"{tabname}_ext_an_model_path_{i+1}", interactive=False, visible=False)
                        model.change(
                        ...
                        with gr.Column() as col:
                            weight = gr.Slider(elem_id=f"{tabname}_ext_an_weight_{i+1}",label=f"Weight {i+1}", value=1.0, minimum=-1.0, maximum=2.0, step=0.05, visible=True)
                            weight_unet = gr.Slider(
                                elem_id=f"{tabname}_ext_an_unet_weight_{i+1}",label=f"UNet Weight {i+1}", value=1.0, minimum=-1.0, maximum=2.0, step=0.05, visible=False
                            )
                            weight_tenc = gr.Slider(
                                elem_id=f"{tabname}_ext_an_tenc_weight_{i+1}",label=f"TEnc Weight {i+1}", value=1.0, minimum=-1.0, maximum=2.0, step=0.05, visible=False
                            )
                ...
                with gr.Accordion("Extra args", open=False):
                    with gr.Row():
                        mask_image = gr.Image(label="mask image:",elem_id=f"{tabname}_ext_an_mask_image")
